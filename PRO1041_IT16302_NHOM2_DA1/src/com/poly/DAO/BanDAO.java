...
public class BanDAO extends CoffeShopSysDAO<Ban, String> {
    ...
    String INSERT_SQL = "INSERT INTO BAN (MaBan, TenBan, MaKV, GhepBan, DeleteBan) VALUES (?, ?, ?, ?, ?)";
    String UPDATE_SQL = "UPDATE BAN SET TenBan=?, MaKV=?, GhepBan=? WHERE MaBan=?";
    String UPDATE_GB = "UPDATE BAN SET GhepBan=? WHERE MaBan=?";
    String DELETE_SQL = "UPDATE BAN SET DeleteBan=1 WHERE MaBan=?";
    String SELECT_ALL_SQL = "SELECT * FROM BAN WHERE DeleteBan=0";
    String SELECT_ALLNew_SQL = "SELECT * FROM BAN";
    String SELECT_BY_ID_SQL = "SELECT * FROM BAN WHERE MaBan=? AND DeleteBan=0";
    String SELECT_BY_MaBan = "SELECT TenBan FROM BAN WHERE MaBan=? AND DeleteBan=0";
    ...
    public List<Ban> findByIdKhuVuc(String makv) {
        String sql = "SELECT * FROM BAN where MaKV='" + makv + "' AND DeleteBan=0";
