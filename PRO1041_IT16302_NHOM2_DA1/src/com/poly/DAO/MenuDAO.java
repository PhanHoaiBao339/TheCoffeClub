...
public class MenuDAO extends CoffeShopSysDAO<Menu, String> {
    ...
    String INSERT_SQL = "INSERT INTO MENU (MaMon, TenMon, Gia, HinhAnh, MaLoai, DeleteMon) VALUES (?, ?, ?, ?, ?, ?)";
    String UPDATE_SQL = "UPDATE MENU SET TenMon=?, Gia=?, HinhAnh=?, MaLoai=? WHERE MaMon=?";
    String DELETE_SQL = "UPDATE MENU SET DeleteMon=1 WHERE MaMon=?";
    String SELECT_ALL_SQL = "SELECT * FROM MENU WHERE DeleteMon=0";
    String SELECT_BY_ID_SQL = "SELECT * FROM MENU WHERE MaMon=? And DeleteMon=0";
    String SELECT_BY_KEYWORD_SQL = "SELECT * FROM MENU WHERE DeleteMon=0 AND (TenMon LIKE ? OR MaLoai LIKE ?)";
