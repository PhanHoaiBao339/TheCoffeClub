...
//                }
    ...
    private void clickTblLeft() {
        Ban bT = daoban.selectById(txtMaBan.getText());
        Ban bP = daoban.selectById(cboChuyenGhep.getSelectedItem().toString().split("-")[1]);
        if (bT.getGhepBan().equals("") && bP.getGhepBan().equals("")) {
    ...
    private void chuyenDoiBan() {
        if (txtMaHoaDonBanChuyen.getText().equals("Bàn Mới")) {
            ...
            Ban bP = daoban.selectById(hd.getMaBan());

            if (!bT.getGhepBan().equals("")) {
                String[] Bgs = bT.getGhepBan().split("-");
                for (String Bg : Bgs) {
                    if (!Bg.equals("")) {
                        Ban b2 = daoban.selectById(Bg);
                        b2.setGhepBan(bP.getMaBan());
                        daoban.update(b2);
                    }
                }

                bP.setGhepBan(bT.getGhepBan());
                daoban.update(bP);
    ...
    private void gopGhepBan() {
        if (tblHoaDonGoc.getRowCount() == 0 && tblHoaDonDaChuyen.getRowCount() == 0) {
            ...
            if (txtMaHoaDonBanChuyen.getText().equals("Bàn Mới")) {
                ...
                Ban b1 = daoban.selectById(txtMaBan.getText());
                String ghepban = b1.getGhepBan() + "-" + hd2.getMaBan();
                daoban.updateGB(ghepban, hd1.getMaBan());
