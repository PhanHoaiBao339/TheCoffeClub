...
public class ThongKeJDialog extends javax.swing.JDialog {
    ...
    private void Sort() {
        ...
        try {
            int month = now.getMonthValue();
            int day = now.getDayOfMonth();
            List<HoaDon> listHD = tkdao.selectAll(day, month, year);
            switch (cboSort.getSelectedIndex()) {
                case 1:
                    Collections.sort(listHD, (HoaDon o1, HoaDon o2) -> o1.getTenNV().compareTo(o2.getTenNV()));
                    break;
                case 2:
                    Collections.sort(listHD, (HoaDon o1, HoaDon o2) -> o1.getTongTien() < o2.getTongTien() ? 1 : -1);
                    break;
                default:
                    Collections.sort(listHD, (HoaDon o1, HoaDon o2) -> o1.getMaHD() > o2.getMaHD() ? 1 : -1);
                    break;
            }
    ...
    private void showTenNV() {
        List<HoaDon> listhdtk = tkdao.selectByTenNV();
        for (HoaDon nv : listhdtk) {
    ...
    private void showHDTenNV() {
        ...
        if (TenNV.equals("Tất cả")) {
            ...
            int i = 1;
            try {
                List<HoaDon> listTK = tkdao.selectListByTenNV(TenNV);
    ...
    private void ShowTheoKhoangTien() {
        ...
        if (money.equals("Tất cả")) {
            ...
            try {
                List<HoaDon> listHD = tkdao.selectByKhoangTien(moneyt[0], moneyt[1]);
                for (HoaDon hd : listHD) {
                    Object[] row = {
                        i,
                        hd.getMaHD(),
    ...
    private void initComponents() {
        ...
        cboSoTien.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Tất cả", "0-100000", "100000-200000", "200000-500000", "500000-5000000" }));
    ...
    private void btnXuatExcel1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatExcel1ActionPerformed
        try {
            this.exportExcelFile(tblHoaDon, "Thông kê hóa đơn ngày " + dateformat.format(new Date()));
//            this.exportExcelFile(tblHoaDon, "Thông kê hóa đơn ngày " + tblHoaDon.getValueAt(0, 2));
        } catch (Exception e) {
            new ThongBaoJDialog(null, true).alert(2, "Xuất Excel thất bại, hóa đơn đang trống!");
        }
